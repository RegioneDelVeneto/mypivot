...
import { manageError, User, UserService } from 'projects/mypay4-fe-common/src/public-api';
  ...
  private onUserChanged(thisRef:HeaderComponent, user:User){
    if(user == null) {
      ...
      if(!user.emailValidationNeeded)
        thisRef.enteService.getEntiByUtente().subscribe(enti => {
          ...
          if(error?.status === 401){
            ...
            this.router.navigate(['not-authorized'],{state:{errorUID: errorUID}});
          } else
